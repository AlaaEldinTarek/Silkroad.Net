...
public class Session : IDisposable {
    ...
    private void Dispose(bool disposing) {
        ...
        if (disposing) this._socket.Dispose();
    }
    ...
    public void RegisterService<T>(T service) where T : class {
        if (this.FindService<T>() != null) return;
    ...
    public void RegisterHandler(MessageHandler handler) {
        if (this._handlers.FirstOrDefault(x => x.Item2 == handler) != null) return;
    ...
    public async Task<Message> ReceiveAsync() {
        ...
        while (true) {
            ...
            if (msg.ID.Equals(MessageID.MASSIVE)) {
                ...
                if (isHeader) {
                    if (massiveMsg != null) throw new InvalidMessageException(InvalidMessageReason.Distorted);
    ...
    public async Task HandshakeAsync() {
        ...
        if (server != null) await server.Begin(this).ConfigureAwait(false);
